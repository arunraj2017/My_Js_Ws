...
Input Explanation: The intersected node's value is 8 (note that this must 
    not be 0 if the two lists intersect). From the head of A, it reads as [4,1,8,4,5]. 
...
function getIntersectedNode(node_a, node_b) {
    if (node_a == null || node_b == null) {
        return null;
    }
    reverseLinkedList(node_a);
    reverseLinkedList(node_b);
    while(node_a.next!=null && node_b.next!=null) {
        if(node_a.val != node_b.val) {
            return null;
        }
        if(node_a.next.val !=== node_b.next.val) {
            return node_a.val;
        }
    }
...
function reverseLinkedList(node) {
    ...
    let nd = reverseLinkedList(node.next);
    node.next.next = node;
    node.next = null;
    return nd;
}

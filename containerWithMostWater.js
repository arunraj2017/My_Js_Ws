/**
 * Given n non-negative integers a1, a2, ..., an , w
 * here each represents a point at coordinate (i, ai). n vertical 
 * lines are drawn such that the two endpoints of line i is at (i, ai) and (i, 0).
 *  Find two lines, which together with x-axis forms a container, such that the
 *  container contains the most water.

Note: You may not slant the container and n is at least 2.


Input: [1,8,6,2,5,4,8,3,7]
Output: 49
 */


 function findMaxContainer (nums) {
     if(nums == null || nums.length<3) {
         return 0;
     }
     let left=0, right=nums.length-1, max=0;
     while(left<right) {
        let tempMin = Math.min(nums[left], nums[right]) * (left - right);
        max = Math.max(Math.abs(tempMin), max);

        if(nums[left] < nums[right]) {
            left++;
        }else{
            right--;
        }
          
     }
     return max;
 }

 console.log(findMaxContainer([1,8,6,2,5,4,8,3,7]));